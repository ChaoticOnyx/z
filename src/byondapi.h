...
/*
	BYOND public API version 516.1674
	...
	- BYOND 516.1674:
	  ...
	  - Temporary refs have been abolished. All references are persistent.
	  - The ByondValue class attaches (accepts ownership without incref) a ref
	    in its CByondValue constructor.

	  - For thread safety, Byond_LastError() no longer takes zero arguments
	    and returns a char pointer. Instead, it requires you to pass in a
	    char* buffer and a u4c pointer for the buffer length, like
	    Byond_ToString() uses. Returns true if assignment was successful,
	    false if more room is needed in the buffer. The buffer length needed
	    is assigned to the length pointer in either case.

	  - ByondExtException is no longer thrown unless a thread-local var is set
	    to signal it should be thrown.
 ...
 * Note: This function is thread-safe, but there is no guarantee the error message can't be changed by another thread between calls.
 * Best practice is to use a while() loop to call this function, reallocating the buffer on failure, until success.
 * @param buf char array, allocated by caller (can be null if querying length)
 * @param buflen Pointer to length of array in bytes; receives the string length (including trailing null)
 * @return True on success; false if array is not big enough
 */
DUNGPUB bool Byond_LastError(char *buf, u4c *buflen);
 ...
 */
DUNGPUB void Byond_SetLastError(char const *err);
...
/**
 * @param v Pointer to CByondValue
 * @return Floating point number for v, or NaN if not numeric
 */
 ...
 * @return True if values are equal
 */
DUNGPUB bool ByondValue_Equiv(CByondValue const *a, CByondValue const *b);
 ...
 * Blocks if not on the main thread.
 * @param result Result
 * @param len Length of new list
 * @return True on success
 */
DUNGPUB bool Byond_CreateListLen(CByondValue *result, u4c len);

/**
 * Creates a multi-dimensional list (referenced) with a given size. Equivalent to new/list(sizes[0],sizes[1],...).
 * Blocks if not on the main thread.
 * @param result Result
 * @param sizes Length of each dimension
 * @param dimension Number of dimensions
 * @return True on success
 */
DUNGPUB bool Byond_CreateDimensionalList(CByondValue *result, u4c const *sizes, u4c dimension);
 ...
 * The length of the array is always a multiple of 2.
 * Blocks if not on the main thread. 
 * @param loc The list to read
 * @param list CByondValue array, allocated by caller (can be null if querying length)
 * @param len Pointer to length of array (in items); receives the number of items read on success, or required length of array if not big enough
 ...
 * Note: Unlike in DM, if one of the items to remove is a list itself, this will treat it as an item instead of looking at the list's contents.
 * This call will never fail. It will simply return 0 if nothing was removed.
 * Blocks if not on the main thread. 
 * @param loc The list
 ...
 */
DUNGPUB u4c Byond_ListRemove(CByondValue const *loc, CByondValue const *items, u4c count);
